...
tab1, tab2, tab3, tab4 = st.tabs(["📊 Season Simulator", "🔮 Match Predictor", "📈 Heatmap", "📊 Team Insights"])
...
# Load detailed dataset and prepare team summary stats for Team Insights tab
df_detailed = pd.read_csv("matches_full.csv")

summary_df = df_detailed.groupby('team').agg({
    'gf': 'mean',
    'ga': 'mean',
    'xg': 'mean',
    'xga': 'mean',
...
summary_df['penalty_conversion'] = summary_df['pk'] / summary_df['pkatt']
summary_df['penalty_conversion'].fillna(0, inplace=True)
...
with tab4:
    ...
    st.markdown("Interactive visualizations from the extended dataset (matches_full.csv)")

    # Sidebar filter for selecting metric
    ...
    metric_column_map = {
        "Average Goals For (gf)": "gf",
        "Average Goals Against (ga)": "ga",
    ...
    selected_col = metric_column_map[metric]

    fig = px.bar(
        summary_df.sort_values(by=selected_col, ascending=False),
        x="team",
        y=selected_col,
        title=f"{metric} by Team",
        labels={"team": "Team", selected_col: metric},
        color="team",
